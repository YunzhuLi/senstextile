...
if args.eval == 0:
    if args.resume == 1:
        ...
        model_path = os.path.join(args.ckp_path, model_name)
        print("Loading kuka network from %s" % model_path)
        model_kuka.load_state_dict(torch.load(model_path))
    else:
        print("Randomly initialize the kuka network")


if args.eval == 1:
    if args.epoch < 0:
        model_kuka_name = 'net_kuka_best.pth'
    else:
        model_kuka_name = 'net_kuka_epoch_%d_iter_%d.pth' % (args.epoch, args.iter)

    model_path = os.path.join(args.ckp_path, model_kuka_name)
    print("Loading kuka network from %s" % model_path)
    model_kuka.load_state_dict(torch.load(model_path))

    plt.rcParams["figure.figsize"] = (6, 6)
    plt.imshow(model_kuka.position_bias.detach().numpy()[0, 0])
    plt.colorbar()
...
for phase in phases:
    datasets[phase] = KnittedVestDataset(
        args, vis_glove.mask, vis_kuka.mask, phase=phase)
...
for epoch in range(n_epoch):
    ...
    for phase in phases:
        ...
        if args.eval == 1:
            ...
            touch_kuka_raw_rec, touch_kuka_rec, touch_kuka_cal_rec = \
                    np.zeros((len(datasets[phase]), 32, 32)), \
                    np.zeros((len(datasets[phase]), 32, 32)), \
                    np.zeros((len(datasets[phase]), 32, 32))
        ...
        for i, data in bar(enumerate(dataloaders[phase], 0)):
            ...
            if args.knit_name in ['kuka_calibration']:
                ...
                touch_glove_raw, touch_glove, touch_kuka_raw, touch_kuka = data

                with torch.set_grad_enabled(phase == 'train'):
                    glove_cal, glove_recon = model_glove(touch_glove)
                    kuka_cal, kuka_recon = model_kuka(touch_kuka)

                min_value = min(min_value, kuka_recon.min().item())
                max_value = max(max_value, kuka_recon.max().item())

                glove_scale = torch.mean(glove_cal, (1, 2, 3))
                kuka_scale = torch.mean(kuka_cal, (1, 2, 3))

                loss_scale = F.mse_loss(glove_scale, kuka_scale)
                loss_recon = F.l1_loss(kuka_recon[:, 0], touch_kuka[:, args.obs_window//2])
            ...
            if args.eval == 1:
                ...
                touch_kuka_raw_cur = to_np(touch_kuka_raw)
                touch_kuka_cur = to_np(touch_kuka[:, args.obs_window // 2])
                touch_kuka_cal_cur = to_np(kuka_recon[:, 0])
                ...
                touch_kuka_raw_rec[st:st + B] = touch_kuka_raw_cur
                touch_kuka_rec[st:st + B] = touch_kuka_cur
                touch_kuka_cal_rec[st:st + B] = touch_kuka_cal_cur
                ...
                if args.vis == 1:
                    for idx_frame in range(B):
                        ...
                        touch_kuka_raw_render = vis_kuka.render(
                            touch_kuka_raw_cur[idx_frame], lim_low=500, lim_high=800, text='raw')
                        touch_kuka_render = vis_kuka.render(
                            touch_kuka_cur[idx_frame], lim_low=-2, lim_high=27, text='selfsupervised')

                        frame = vis_glove.merge_frames(
                            [touch_glove_raw_render, touch_glove_render,
                             touch_kuka_raw_render, touch_kuka_render], nx=2, ny=2)
            ...
            if i > 0 and i % args.ckp_per_iter == 0 and phase == 'train':
                model_path = '%s/net_kuka_epoch_%d_iter_%d.pth' % (args.ckp_path, epoch, i)
                torch.save(model_kuka.state_dict(), model_path)
        ...
        print('[%d/%d %s] min_value: %.4f, max_value: %.4f, gBias: %.4f, gScale: %.4f' % (
            epoch, args.n_epoch, phase, min_value, max_value, model_kuka.global_bias, model_kuka.global_scale))
        ...
        if phase == 'valid' and not args.eval:
            ...
            if loss_cur < best_valid:
                best_valid = loss_cur
                torch.save(model_kuka.state_dict(), '%s/net_kuka_best.pth' % (args.ckp_path))
        ...
        if args.eval == 1:
            ...
            print('touch_kuka_raw_rec', touch_kuka_raw_rec.shape)
            print('touch_kuka_rec', touch_kuka_rec.shape)
            print('touch_kuka_cal_rec', touch_kuka_cal_rec.shape)

            touch_glove_cal_sum = np.sum(touch_glove_cal_rec, (1, 2))

            touch_kuka_raw_sum = np.sum(touch_kuka_raw_rec, (1, 2))
            touch_kuka_sum = np.sum(touch_kuka_rec, (1, 2))
            touch_kuka_cal_sum = np.sum(touch_kuka_cal_rec, (1, 2))

            # remove outliers
            mask_inliers = np.logical_not(touch_glove_cal_sum > 1000)
            touch_glove_cal_sum = touch_glove_cal_sum[mask_inliers]
            touch_kuka_raw_sum = touch_kuka_raw_sum[mask_inliers]
            touch_kuka_sum = touch_kuka_sum[mask_inliers]
            touch_kuka_cal_sum = touch_kuka_cal_sum[mask_inliers]
            ...
            cor_raw = np.corrcoef(touch_glove_cal_sum, touch_kuka_raw_sum)[0, 1]
            cor_hand = np.corrcoef(touch_glove_cal_sum, touch_kuka_sum)[0, 1]
            cor_cal = np.corrcoef(touch_glove_cal_sum, touch_kuka_cal_sum)[0, 1]
            ...
            plt.legend(loc='upper left', fontsize=20, fancybox=True, framealpha=0.2)
            ...
            X_raw = touch_kuka_raw_sum.reshape(-1, 1)
            X_hand = touch_kuka_sum.reshape(-1, 1)
            X_cal = touch_kuka_cal_sum.reshape(-1, 1)
            ...
            # touch_raw
            if len(touch_kuka_raw_sum) > 0:
                plt.rcParams["figure.figsize"] = (6, 6)
                plt.figure()
                plt.plot(touch_glove_cal_sum, touch_kuka_raw_sum, 'bo', markersize=2)
                plt.savefig(os.path.join(args.vis_path, 'touch_correlation_raw.png'))
                plt.close()

                print("touch_raw corrcoef", np.corrcoef(touch_glove_cal_sum, touch_kuka_raw_sum)[0, 1])
                ...
                plt.subplot(212)
                plt.plot(touch_kuka_raw_sum[:n_frames], 'b-')
                plt.savefig(os.path.join(args.vis_path, 'touch_overtime_raw.png'))
                plt.close()

            # touch
            if len(touch_kuka_sum) > 0:
                plt.rcParams["figure.figsize"] = (6, 6)
                plt.figure()
                plt.plot(touch_glove_cal_sum, touch_kuka_sum, 'bo', markersize=2)
                plt.savefig(os.path.join(args.vis_path, 'touch_correlation_hand.png'))
                plt.close()

                print("touch_hand corrcoef", np.corrcoef(touch_glove_cal_sum, touch_kuka_sum)[0, 1])
                ...
                plt.plot(touch_glove_cal_sum[:n_frames], 'r-')
                plt.subplot(212)
                plt.plot(touch_kuka_sum[:n_frames], 'b-')
                plt.savefig(os.path.join(args.vis_path, 'touch_overtime_hand.png'))
                plt.close()

            # touch_cal
            if len(touch_kuka_cal_sum) > 0:
                plt.rcParams["figure.figsize"] = (6, 6)
                plt.figure()
                plt.plot(touch_glove_cal_sum, touch_kuka_cal_sum, 'bo', markersize=2)
                plt.savefig(os.path.join(args.vis_path, 'touch_correlation_cal.png'))
                plt.close()

                print("touch_cal corrcoef", np.corrcoef(touch_glove_cal_sum, touch_kuka_cal_sum)[0, 1])
                ...
                plt.plot(touch_glove_cal_sum[:n_frames], 'r-')
                plt.subplot(212)
                plt.plot(touch_kuka_cal_sum[:n_frames], 'b-')
